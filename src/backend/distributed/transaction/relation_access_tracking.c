...
static void AssociateShardAccessWithRelation(uint64 shardId,
...
/*
 * AssociateShardAccessWithRelation associates the shard access to the
 * distributed relation that the shard belongs to.
 *
 * We implemented this function to prevent accessing placement metadata during
 * recursive calls of the function itself.
 */
static void
AssociateShardAccessWithRelation(uint64 shardId, ShardPlacementAccessType accessType)
{
	...
	if (PartitionedTableNoLock(relationId))
	{
		...
		foreach(partitionCell, partitionList)
		{
			...
			AssociateShardAccessWithRelation(partitionShardId, accessType);
		}
	}
